...
def draw_mask_on_frame(frame, mask_coordinates):
    for i in range(len(mask_coordinates) - 1):
        start_point = tuple(mask_coordinates[i])
        end_point = tuple(mask_coordinates[i + 1])
        cv2.line(frame, start_point, end_point, (0, 255, 0), 2)  # Green color line

    cv2.line(frame, tuple(mask_coordinates[-1]), tuple(mask_coordinates[0]), (0, 255, 0), 2)
    return frame #Added by Garthi to draw mask
